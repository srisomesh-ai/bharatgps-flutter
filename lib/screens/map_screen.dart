...
class _MapScreenState extends State<MapScreen> with TickerProviderStateMixin {
  ...
  // ===== User's own GPS location =====
  Future<void> _goToMyLocation() async {
    Haptics.medium();
    try {
      if (!await Geolocator.isLocationServiceEnabled()) {
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please turn on GPS/location on your phone')));
        return;
      }
  ...
  void _openDetail(Map<String, dynamic> u) {
    showModalBottomSheet(
      ...
      builder: (_) => _VehicleDetailSheet(
        device: u,
        // Show Engine Cut-Off ONLY if this device has the 'blocked' sensor
        // (logical template) configured — i.e. the engine-cut relay is set up.
        supportsCutoff: u['hasCutSensor'] == true,
      ),
    );
  }
